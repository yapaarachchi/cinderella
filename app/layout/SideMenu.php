...
foreach($Category->getMainCategory() as $key => $value) {
	...
	<?php
	foreach($Category->getSubCategory($value['id']) as $key => $value) {
	?>
	<a href="app/Search/" style="font-size: 0.90rem; padding: 0.50rem;" class="list-group-item list-group-item-action" > > <?php echo $value['category2'];?></a>
	<?php
	}
	?>
	</div>
